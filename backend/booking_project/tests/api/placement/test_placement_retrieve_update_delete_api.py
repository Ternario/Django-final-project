...
from booking_project.models import Placement, PlacementImage, PlacementDetails, PlacementLocation, User
...
class PlacementRetrieveUpdateDeleteTest(PlacementSetup):
    ...
    def test_unauthenticated_user_can_retrieve_full_info_about_placement(self):
        ...
        self.assertEqual(response.data["placement_image"][0]["placement"], self.placement.pk)
        self.assertEqual(len(response.data["placement_image"]), len(self.placement_images))
    ...
    def test_user_owner_can_delete_placement(self):
        ...
        self.assertFalse(PlacementDetails.objects.filter(placement=self.placement.pk))
        self.assertFalse(PlacementLocation.objects.filter(placement=self.placement.pk))
    ...
    def test_unauthorized_owner_user_cannot_delete_placement(self):
        ...
        self.assertEqual(PlacementImage.objects.filter(placement=self.placement.pk).count(), 8)
        self.assertTrue(PlacementDetails.objects.get(placement=self.placement.pk))
        self.assertTrue(PlacementLocation.objects.get(placement=self.placement.pk))
