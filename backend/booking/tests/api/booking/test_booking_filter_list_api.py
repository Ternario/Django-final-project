...
from booking.models.choices import CheckInTime, CheckOutTime
...
class BookingFilterTest(BookingSetup):
    ...
    def test_landlord_user_can_retrieve_booking_list_his_placements(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), Booking.objects.filter(placement__owner=self.landlord_user2).count())
    ...
    def test_user_can_retrieve_list_his_bookings(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), Booking.objects.filter(user=self.landlord_user).count())
    ...
    def test_user_can_retrieve_list_his_bookings_by_query_params(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data),
                         Booking.objects.filter(user=self.landlord_user, status="CONFIRMED").count())
