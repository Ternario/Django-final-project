...
from properties.serializers.user import UserDetailSerializer
from properties.tests.api.user.user_setup import UserApiTests
...
class UserLoginLogoutTests(UserApiTests):
    ...
    def test_user_retrieve_correct_info_to_invalid_email(self):
        """
        Test if the user receive correct answer to invalid email.
        Sends a POST request and expects a 401 UNAUTHORIZED response.
        Verifies that the user does not receive cookies and receive correct answer.
        """
    ...
    def test_deleted_user_cannot_login(self):
        ...
        Sends a POST request with login data and expects a 401 UNAUTHORIZED response.
