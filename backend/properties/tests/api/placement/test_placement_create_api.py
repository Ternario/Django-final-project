...
from properties.models import Placement, PlacementDetails, PlacementLocation, PlacementImage
...
class PlacementCreateTest(PlacementSetup):
    def test_landlord_user_can_create_placement(self):
        ...
        self.assertEqual(Placement.not_deleted_objects.count(), 3)
        self.assertEqual(PlacementDetails.related_objects.count(), 3)
        self.assertEqual(PlacementLocation.related_objects.count(), 3)
        self.assertEqual(PlacementLocation.related_objects.last().placement,
                         Placement.not_deleted_objects.get(id=first_response.data["id"]))
        self.assertEqual(PlacementDetails.related_objects.last().placement,
                         Placement.not_deleted_objects.get(id=first_response.data["id"]))
        ...
        self.assertTrue(Placement.not_deleted_objects.get(pk=first_response.data["id"]).is_active)
        self.assertEqual(PlacementImage.related_objects.count(), 31)
        self.assertEqual(PlacementImage.related_objects.filter(placement=first_response.data["id"]).count(), 15)
    ...
    def test_landlord_user_can_create_new_placement_and_cannot_add_more_then_fifteen_images(self):
        ...
        self.assertEqual(PlacementImage.related_objects.count(), 16)
        self.assertTrue(Placement.inactive_objects.get(pk=first_response.data["id"]))
    ...
    def test_landlord_user_cannot_create_new_placement_without_category(self):
        ...
        self.assertEqual(Placement.not_deleted_objects.count(), 2)
        self.assertEqual(PlacementLocation.related_objects.count(), 2)
        self.assertEqual(PlacementDetails.related_objects.count(), 2)
