...
from properties.models import Placement, PlacementLocation
...
class PlacementSearchTest(PlacementSetup):
    ...
    def test_user_retrieve_active_placements_list(self):
        ...
        self.assertEqual(len(response.data), Placement.active_objects.all().count())
    ...
    def test_user_retrieve_active_placements_list_by_text(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), Placement.active_objects.filter(
    ...
    def test_user_retrieve_active_placements_list_by_city(self):
        """
    ...
    def test_user_retrieve_active_placements_list_by_category(self):
        """
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data),
                         Placement.active_objects.filter(category__name__in=["Hotels"]).count())

    def test_user_retrieve_active_placements_list_by_details(self):
        """
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data),
                         Placement.active_objects.filter(placement_details__pets=True,
                                                         placement_details__room_service=True,
                                                         placement_details__balcony=True,
                                                         placement_details__coffee_tee_maker=True).count())

    def test_user_retrieve_active_placements_list_by_rooms_number(self):
        """
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data),
                         Placement.active_objects.filter(number_of_rooms__in=[2]).count())

    def test_user_retrieve_active_placements_list_by_price_range(self):
        """
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data),
                         Placement.active_objects.filter(price__gte=200, price__lte=400).count())

    def test_user_retrieve_active_placements_list_by_multy_params(self):
        """
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data),
                         Placement.active_objects.filter(placement_location__city__in=["Test first city"],
                                                         price__gte=200,
                                                         price__lte=400, number_of_rooms__in=[2],
                                                         placement_details__pets=True,
                                                         placement_details__room_service=True,
                                                         placement_details__balcony=True,
                                                         placement_details__coffee_tee_maker=True).count())
