...
from properties.serializers.base_data import DiscountBaseSerializer, PropertyOwnerBaseSerializer
from properties.utils.choices.discount import (
    DiscountValueType, DiscountUserStatus, DiscountType, DiscountStatus, DiscountPropertyStatus
)
...
class DiscountPropertyCreateSerializer(ModelSerializer):
    class Meta:
        ...
        fields = ['id', 'discount', 'property_ref', 'landlord_profile', 'added_by', 'status']
    ...
    def validate(self, attrs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if landlord_profile.pk != property_ref.owner_id:
            raise ValidationError({'non_field_errors': DISCOUNT_PROPERTY_ERRORS['owner']})

        non_field_errors: List[str] = []

        if discount.status not in [DiscountStatus.ACTIVE.value[0], DiscountStatus.SCHEDULED.value[0]]:
            non_field_errors.append(DISCOUNT_PROPERTY_ERRORS['discount_not_available'])

        if (
                discount.status != DiscountStatus.ACTIVE.value[0] and status ==
                DiscountPropertyStatus.ACTIVE.value[0]
        ):
            non_field_errors.append(DISCOUNT_PROPERTY_ERRORS['discount_not_active'])

        if (
                discount.status == DiscountStatus.SCHEDULED.value[0] and status !=
...
class DiscountPropertyBaseSerializer(ModelSerializer):
    ...
    class Meta:
        model = DiscountProperty
        fields = ['id', 'discount', 'property', 'status']


class DiscountPropertySerializer(ModelSerializer):
    discount = DiscountPublicSerializer(read_only=True)
    property = PropertyOwnerBaseSerializer(source='property_ref', read_only=True)
    ...
    def update(self, instance: DiscountProperty, validated_data: Dict[str, Any]) -> DiscountProperty:
        status: str | None = validated_data.pop('status', None)
        removed_by: User = self.context['user']

        if status == DiscountPropertyStatus.REMOVED.value[0]:
            instance.set_removed(removed_by)
        else:
            instance.set_active()
    ...
    def validate(self, attrs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        discount: Discount = attrs.get('discount')

        status: str | None = attrs.get('status', None)

        if instance.status == DiscountPropertyStatus.ACTIVE.value[0] and instance.status == status:
            raise ValidationError({'non_field_error': DISCOUNT_ERRORS['already_active']})

        if instance.status == DiscountPropertyStatus.REMOVED.value[0] and instance.status == status:
            raise ValidationError({'non_field_error': DISCOUNT_ERRORS['already_inactive']})

        if (
                instance.status == DiscountPropertyStatus.EXPIRED.value[0] and status in
                [DiscountPropertyStatus.ACTIVE.value[0], DiscountPropertyStatus.REMOVED.value[0]]
        ):
            raise ValidationError({'status': DISCOUNT_PROPERTY_ERRORS['status']})

        if discount.status != DiscountStatus.ACTIVE.value[0] and status == DiscountPropertyStatus.ACTIVE.value[0]:
            raise ValidationError({'non_field_error': DISCOUNT_PROPERTY_ERRORS['discount_not_active']})
...
class DiscountUserBaseSerializer(ModelSerializer):
    discount_name = DiscountBaseSerializer(source='discount', read_only=True)
    status = CharField(source='get_status_display', read_only=True)
...
class DiscountUserPropertyOwnerSerializer(ModelSerializer):
    ...
    def update(self, instance: DiscountUser, validated_data: Dict[str, Any]) -> DiscountUser:
        removed_by: User = self.context['user']
        remove_reason: str = validated_data.get('remove_reason')
